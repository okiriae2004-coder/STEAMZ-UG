...
export const DishModal: React.FC<DishModalProps> = ({
  ...
  const unitPrice =
    item.price + additionalCost;

  const totalPrice =
    unitPrice * quantity;

  const handleSelectSingleChoice = (
    optionName: string,
    choiceName: string
  ) => {
    setValidationError(null);
    ...
    setSelectedOptions(
      (prev) => {
        ...
        if (
          ...
          setValidationError(
            `You can only select up to ${maxSelect} choices for "${optionName}"`
          );
      ...
      for (const opt of item.options) {
        if (opt.required) {
          ...
          if (
            ...
            setValidationError(
              `Please make a selection for "${opt.name}" before proceeding.`
            );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 backdrop-blur-xs p-3 sm:p-4 animate-in fade-in duration-200">
      ...
      <div className="w-full max-w-lg max-h-[92vh] flex flex-col rounded-3xl bg-white shadow-2xl border border-stone-200 overflow-hidden">
        ...
        <div className="relative shrink-0 overflow-hidden bg-gradient-to-br from-stone-900 via-stone-800 to-stone-950 px-4 py-4 sm:px-5 sm:py-5">

          <div className="flex items-center gap-3 pr-12">

            <div className="h-12 w-12 rounded-xl bg-amber-500/15 border border-amber-400/20 flex items-center justify-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 rounded-full bg-stone-900/70 p-2 text-white hover:bg-stone-900 transition shadow-md"
            aria-label="Close"
          >
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">
            ...
            <span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-bold text-stone-200 border border-white/10 flex items-center gap-1.5">
            ...
            {item.prepTimeMinutes && (
              <span className="rounded-full bg-white/10 px-2 py-1 text-[11px] font-medium text-stone-200 border border-white/10">
                ~{item.prepTimeMinutes}m
              </span>
            )}

            {item.portionSize && (
              <span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-medium text-stone-300 border border-white/10 flex items-center gap-1">
